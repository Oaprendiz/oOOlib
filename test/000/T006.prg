// hbmk2 T006 xmlparse.c hxmldoc.prg myxml.prg -b hbmzip.hbc
#include "fileio.ch"
...
METHOD SetTitle(cTitle) CLASS odsMeta
IF len(::oTitle:aItems) == 0
   ::oTitle:Add(cTitle)
ELSE
   ::oTitle:aItems[1] := cTitle
ENDIF
RETURN ::oTitle

//---------------------------------------------------------------------//

METHOD SetDescription(cDescription) CLASS odsMeta
IF len(::oDescription:aItems) == 0
   ::oDescription:Add(cDescription)
ELSE
   ::oDescription:aItems[1] := cDescription
ENDIF
RETURN ::oDescription

//---------------------------------------------------------------------//

METHOD SetSubject(cSubject) CLASS odsMeta
IF len(::oSubject:aItems) == 0
   ::oSubject:Add(cSubject)
ELSE
   ::oSubject:aItems[1] := cSubject
ENDIF
RETURN ::oSubject

//---------------------------------------------------------------------//

METHOD SetCreator(cCreator) CLASS odsMeta
IF len(::oInitCreator:aItems) == 0
   ::oInitCreator:Add(cCreator)
ELSE
   ::oInitCreator:aItems[1] := cCreator
ENDIF
RETURN ::oInitCreator

//---------------------------------------------------------------------//

METHOD SetModifier(cModifier) CLASS odsMeta
IF len(::oCreator:aItems) == 0
   ::oCreator:Add(cCreator)
ELSE
   ::oCreator:aItems[1] := cCreator
ENDIF
RETURN ::oCreator
